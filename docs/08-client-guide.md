...
- Ответ: `{ id, email, fullName, role, isActive, employeeId }`
...
- Побочный эффект (Web): устанавливает cookies `refreshToken` и `XSRF-TOKEN`
...
- CSRF: требуется для браузера
- Тело (Mobile): `{ refreshToken }` — для клиентов без cookie
- Ответ: `{ accessToken, refreshToken }`
- Побочный эффект (Web): ротирует `refreshToken` cookie и `XSRF-TOKEN`
...
- CSRF: требуется для браузера
- Ответ: `{ success: true }`
- Побочный эффект (Web): очищает cookies `refreshToken` и `XSRF-TOKEN`
...
| `EMPLOYEE` | Базовый доступ (только собственные данные) |
...
| `GET` | `/etl/imports/:id` | Детали импорта |
